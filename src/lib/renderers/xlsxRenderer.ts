...
export const generateScheduleXLSX = async (docData: ProductionDocModel) => {
    ...
    docData.days.forEach(day => {
        const sheet = workbook.addWorksheet(`Day ${day.dayNumber}`);
        ...
        sheet.getCell('A2').value = `Day ${day.dayNumber} (${day.date}) | Call: ${day.shootDay.callTime} | Loc: ${day.shootDay.mainLocation.name}`;
    ...
    a.href = url;
    a.download = `Schedule_${docData.project.title}_${docData.days[0].date}.xlsx`;
    a.click();
